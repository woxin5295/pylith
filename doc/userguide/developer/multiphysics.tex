...
The Jacobian for the Generalized Maxwell material is:
\begin{align}
  C_{1112} &= C_{1113} = C_{1121} = C_{1123} = C_{1131} = C_{1132} = 0,\\
  C_{1213} &= C_{1222} = C_{1223} = C_{1231} = C_{1232} = C_{1233} = 0,
\end{align}
...
\subsubsection{Power-Law Maxwell Viscoelasticity}
...
\begin{equation}
  \label{eqn:visco:powerlaw:devstress}
...
\begin{equation}
  \label{eqn:visco:powerlaw:devstrain}
  \sqrt{\dot{L}_{2}^{dev\;C}}=\frac{\sqrt{3}}{2}\dot{\epsilon}_{11}\:.
\end{equation}
Substituting Equations~\ref{eqn:visco:powerlaw:devstress}
and~\ref{eqn:visco:powerlaw:devstrain} into Equation~\ref{eqn:visco:powerlaw:lab}, we obtain
\begin{equation}
  \sqrt{\dot{L}_{2}^{dev\;C}}=A_{E}\frac{\sqrt{3}^{n+1}}{2}\exp\left(\frac{-Q}{RT}\right)\sqrt{J_{2}^{dev}}^{n}\:,
\end{equation}
and therefore,
\begin{equation}
...
\begin{equation}
  \label{eqn:visco:powerlaw:AT}
  A_{T}=A_{M}\exp\left(\frac{-Q}{RT}\right)=\frac{\sqrt{3}^{n+1}}{2}A_{E}\exp\left(\frac{-Q}{RT}\right)\:.
\end{equation}

\charles{We are missing a bunch of introductory material for the
equations below. Once this is done properly, we should probably have
an introductory section for viscoelastic materials (or maybe a section
for all materials) where we define a bunch of parameters and methods
(e.g., stress and strain rate invariants, alpha method of time
integration, etc.).}
...
\begin{equation}
  \frac{\sqrt{\dot{L}_{2}^{dev\;C}}}{\dot{\epsilon}_{0}^{dev}}=\left(\frac{\sqrt{J_{2}^{dev}}}{\sigma_{0}^{dev}}\right)^{n},
\end{equation}
where $\dot{\epsilon}_{0}^{dev}$ and $\sigma_{0}^{dev}$ are reference
values for the deviatoric strain rate and stress. This means that
\begin{equation}
  \frac{\dot{\epsilon}_{0}^{dev}}{\left(\sigma_{0}^{dev}\right)^{n}}=A_{T}\:.
\end{equation}
...
to $\dot{\epsilon}_{0}^{dev}$, $\sigma_{0}^{dev}$, and $n$, respectively. To specify the
power-law properties for PyLith using laboratory results, the user
must first compute $A_{T}$ using Equation~\ref{eqn:visco:powerlaw:AT}. Then, values
for $\dot{\epsilon}_{0}^{dev}$ and $\sigma_{0}^{dev}$ must be provided. The simplest method
is probably to assume a reasonable value for the reference strain
rate, and then compute $\sigma_{0}^{dev}$ as
\begin{equation}
...
utility is described more fully in Section
\vref{sec:example:3dhex8:step08}.

The flow law in component form is 
\begin{equation}
  \label{eqn:visco:powerlaw:flowlawcomponent}
  \dot{\epsilon}_{ij}^{dev\;C}=\frac{\dot{\epsilon}_{0}^{dev}\sqrt{J_{2}^{dev}}^{n-1}\sigma_{ij}^{dev}}{\left(\sigma_{0}^{dev}\right)^{n}}\:,
\end{equation}
and the creep strain increment is approximated as
\begin{equation}
  \label{eqn:visco:powerlaw:creepincrcomponents}
  \Delta {\epsilon}_{ij}^{dev\;C}\approx\frac{\Delta
    t\dot{\epsilon}_{0}^{dev}\sqrt{^{\tau}J_{2}^{dev}}^{n-1}\,^{\tau}{\sigma}_{ij}^{dev}}{\left(\sigma_{0}^{dev}\right)^{n}}=\frac{\Delta
    t\dot{\epsilon}_{0}^{dev}\left(\phantom{}^{\tau}\overline{\sigma}^{dev}\right)^{n-1}\,^{\tau}{\sigma}_{ij}^{dev}}{\sqrt{3}\left(\sigma_{0}^{dev}\right)^{n}}\,
\end{equation}
\charles{This part of the effective stress formulation still bothers
  me. According to the Bathe book, we simultaneously have
  ${}^{\tau}\sigma_{ij}^{dev}=\alpha{}^{t+{\Delta}t}\sigma_{ij}^{dev}+(1-\alpha){}^{t}\sigma_{ij}^{dev}$
  and $\sqrt{{}^{\tau}J_{2}^{dev}} =
  \alpha\sqrt{{}^{t+{\Delta}t}J_{2}^{dev}} +
  (1-\alpha)\sqrt{{}^{t}J_{2}^{dev}}\,.$ This can only be true for
  $\alpha=0$ (forward difference) or $\alpha=1$ (backward
  difference).} 
Therefore,
\begin{equation}
  \label{eqn:visco:powerlaw:creepincrscalar}
  \Delta\bar{\epsilon}^{dev\;C}\approx\frac{2\Delta
    t\dot{\epsilon}_{0}^{dev}\sqrt{^{\tau}J_{2}^{dev}}^{n}}{\sqrt{3}\left(\sigma_{0}^{dev}\right)^{n}}=\frac{2\Delta
    t\dot{\epsilon}_{0}^{dev}\left(\phantom{}^{\tau}\overline{\sigma}^{dev}\right)^{n}}{\sqrt{3}^{n+1}\left(\sigma_{0}^{dev}\right)^{n}}\,,\;\textrm{and}\;^{\tau}\gamma=\frac{\dot{\epsilon}_{0}^{dev}\sqrt{^{\tau}J_{2}^{dev}}^{n-1}}{\left(\sigma_{0}^{dev}\right)^{n}}\,.
\end{equation}
Substituting Equations \charles{Eqn not available yet},
~\ref{eqn:visco:powerlaw:creepincrcomponents}, and
~\ref{eqn:visco:powerlaw:creepincrscalar},
into \charles{Eqn not available yet}, we obtain:
\begin{equation}
  \label{eqn:visco:powerlaw:stresstplusdt}
  ^{t+\Delta t}{\sigma}_{ij}^{dev}=2\mu\left\{
    ^{t+\Delta t}{\epsilon}_{ij}^{\prime\;dev}-\Delta
    t\phantom{}^{\tau}\gamma\left[\left(1-\alpha\right)^{t}{\sigma}_{ij}^{dev}+\alpha{}^{t+\Delta
        t}{\sigma}_{ij}^{dev}\right]\right\}
  +{\sigma}_{ij}^{refdev}\,,
\end{equation}
which may be rewritten:
\begin{equation}
  \label{eqn:visco:powerlaw:effstress}
  ^{t+\Delta t}{\sigma}_{ij}^{dev}\left(\frac{1}{2\mu}+\alpha\Delta
    t\phantom{}^{\tau}\gamma\right)={}^{t+\Delta
    t}{\epsilon}_{ij}^{\prime\;dev}-\Delta
  t\phantom{}^{\tau}\gamma\left(1-\alpha\right)^{t}{\sigma}_{ij}^{dev}+a_{E}{\sigma}_{ij}^{refdev}\,.
\end{equation}
Taking the scalar inner product of both sides we obtain:
\begin{gather}
  \label{eqn:visco:powerlaw:effstressfn}
  a^{2}\,\,{}^{t+\Delta t}J_{2}^{dev}-b+c\phantom{}^{\tau}\gamma-d^{2}\,^{\tau}\gamma^{2}=F=0\,,
\end{gather}
where
\begin{gather}
  a=\frac{1}{2\mu}+\alpha\Delta t\phantom{}^{\tau}\gamma\,\,\nonumber \\
  b=\frac{1}{2}{}^{t+\Delta t}{\epsilon}_{ij}^{\prime\;dev}\cdot
  {}^{t+\Delta t}{\epsilon}_{ij}^{\prime\;dev}+
  \frac{{}^{t+\Delta t}{\epsilon}_{ij}^{\prime\;dev}\cdot
    {\sigma}_{ij}^{refdev}}{2\mu}+\frac{J_{2}^{refdev}}{4\mu^{2}}\,.\\
  c=\Delta t\left(1-\alpha\right){}^{t+\Delta t}{\epsilon}_{ij}^{\prime\;dev}\cdot
  {}^{t}{\sigma}_{ij}^{dev}+
  \frac{\Delta t\left(1-\alpha\right)\,^{t}{\sigma}_{ij}^{dev}\cdot
    {\sigma}_{ij}^{refdev}}{2\mu}\,\,\nonumber
  \\
  d=\Delta t\left(1-\alpha\right)\sqrt{^{t}J_{2}^{dev}}\,\,\nonumber 
\end{gather}
Equation~\ref{eqn:visco:powerlaw:effstressfn} is a function of a
single unknown -- the square root of the second deviatoric stress
invariant at time $t+\Delta t$ -- and may be solved by bisection or by
Newton's method. Once this parameter has been found, the deviatoric
stresses for the current time step may be found from Equations
\charles{Eqn not available yet},
~\ref{eqn:visco:powerlaw:creepincrscalar}, and
~\ref{eqn:visco:powerlaw:stresstplusdt}, and the total stresses may be
found by combining the deviatoric and volumetric components from
Equation \charles{Eqn not available yet}.
...
computing the effective viscous strain rate from
Equation~\ref{eqn:visco:powerlaw:flowlawcomponent}:
\begin{equation}
  \dot{\bar{\epsilon}}^{dev\;C}=\frac{2\dot{\epsilon}_{0}^{dev}\left(\frac{\bar{\sigma}^{dev}}{\sqrt{3}}\right)^{n}}{\sqrt{3}\left(\sigma_{0}^{dev}\right)^{n}}\:.
\end{equation}
Similarly, the effective elastic strain is computed as:
\begin{equation}
  \bar{\epsilon}^{dev\;E}=\frac{\bar{\sigma}^{dev}}{3\mu}\:.
\end{equation}
The relaxation time is then the ratio between these two:
\begin{equation}
  \label{eqn:visco:powerlaw:relaxationtime}
  \tau=\frac{\bar{\epsilon}^{dev\;E}}{\bar{\dot{\epsilon}}^{dev\;C}}=\left(\frac{\sigma_{0}^{dev}}{\sqrt{J_{2}^{dev}}}\right)^{n-1}\frac{\sigma_{0}^{dev}}{6\mu\dot{\epsilon}_{0}^{dev}}\:.
\end{equation}
The stable time step returned by PyLith is 1/5 of the value computed
from Equation~\ref{eqn:visco:powerlaw:relaxationtime}.

To compute the Jacobian for a power-law material, we begin by
rewriting Equation~\ref{eqn:visco:powerlaw:stresstplusdt}:
\begin{equation}
  F=\frac{{}^{t+\Delta t}{\sigma}_{ij}^{dev}}{2\mu} +
  {}^{\tau}\gamma\Delta t{}^{\tau}\sigma_{ij}^{dev}
    -{}^{t+\Delta t}{\epsilon}_{ij}^{\prime\;dev}-
  \frac{{\sigma}_{ij}^{refdev}}{2\mu}=0\,.
\end{equation}
Using implicit differentiation, we can compute the derivatives of the
deviatoric stress with respect to the displacement field. We then add
the volumetric component to obtain the Jacobian components:
\begin{align}
  C_{1112} &= C_{1113} = C_{1121} = C_{1123} = C_{1131} = C_{1132} = 0,\\
  C_{1213} &= C_{1222} = C_{1223} = C_{1231} = C_{1232} = C_{1233} =
             0\,,
\end{align}
and
\begin{align}
  C_{1111} &= K + \frac{2}{3\left[\alpha\Delta t{}^{\tau}\gamma +
             \frac{\alpha\Delta t{}^{\tau}\gamma
             \left(^{\tau}\sigma_{11}^{dev}\right)^{2}\left(n-1\right)}
             {2{}^{\tau}J_{2}^{dev}} + \frac{1}{2\mu}\right]}\,, \\
  C_{2222} &= K + \frac{2}{3\left[\alpha\Delta t{}^{\tau}\gamma +
             \frac{\alpha\Delta t{}^{\tau}\gamma
             \left(^{\tau}\sigma_{22}^{dev}\right)^{2}\left(n-1\right)}
             {2{}^{\tau}J_{2}^{dev}} + \frac{1}{2\mu}\right]}\,, \\
  C_{3333} &= K + \frac{2}{3\left[\alpha\Delta t{}^{\tau}\gamma +
             \frac{\alpha\Delta t{}^{\tau}\gamma
             \left(^{\tau}\sigma_{33}^{dev}\right)^{2}\left(n-1\right)}
             {2{}^{\tau}J_{2}^{dev}} + \frac{1}{2\mu}\right]}\,, \\
  C_{1122} &= K - \frac{2{}^{\tau}J_{2}^{dev}}{3\alpha\Delta t{}^{\tau}\gamma
             {}^{\tau}\sigma_{11}^{dev}{}^{\tau}\sigma_{22}^{dev}
             \left(n-1\right)}\,, \\
  C_{1133} &= K - \frac{2{}^{\tau}J_{2}^{dev}}{3\alpha\Delta t{}^{\tau}\gamma
             {}^{\tau}\sigma_{11}^{dev}{}^{\tau}\sigma_{33}^{dev}
             \left(n-1\right)}\,, \\
  C_{2233} &= K - \frac{2{}^{\tau}J_{2}^{dev}}{3\alpha\Delta t{}^{\tau}\gamma
             {}^{\tau}\sigma_{22}^{dev}{}^{\tau}\sigma_{33}^{dev}
             \left(n-1\right)}\,, \\
  C_{1212} &= \frac{1}{2\left[\alpha\Delta t{}^{\tau}\gamma +
             \frac{\alpha\Delta t{}^{\tau}\gamma
             \left(^{\tau}\sigma_{12}^{dev}\right)^{2}\left(n-1\right)}
             {{}^{\tau}J_{2}^{dev}} + \frac{1}{2\mu}\right]}\,, \\
  C_{1313} &= \frac{1}{2\left[\alpha\Delta t{}^{\tau}\gamma +
             \frac{\alpha\Delta t{}^{\tau}\gamma
             \left(^{\tau}\sigma_{13}^{dev}\right)^{2}\left(n-1\right)}
             {{}^{\tau}J_{2}^{dev}} + \frac{1}{2\mu}\right]}\,, \\
  C_{2323} &= \frac{1}{2\left[\alpha\Delta t{}^{\tau}\gamma +
             \frac{\alpha\Delta t{}^{\tau}\gamma
             \left(^{\tau}\sigma_{23}^{dev}\right)^{2}\left(n-1\right)}
             {{}^{\tau}J_{2}^{dev}} + \frac{1}{2\mu}\right]}\,. \\
\end{align}
